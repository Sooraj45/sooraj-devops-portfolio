...
export default function Home() {
  ...
  return <div className="site-shell min-h-screen overflow-hidden text-slate-100"><div aria-hidden="true" className="pointer-events-none fixed left-4 top-1/2 z-40 hidden -translate-y-1/2 flex-col items-center gap-3 xl:flex"><div className="mb-1 h-10 w-px bg-gradient-to-b from-transparent via-cyan-300/60 to-transparent"/><span className="h-2 w-2 rounded-full bg-cyan-300 shadow-[0_0_15px_rgba(93,228,255,.8)]"/><span className="h-1 w-1 rounded-full bg-slate-600"/><span className="h-1 w-1 rounded-full bg-slate-600"/><span className="h-1 w-1 rounded-full bg-slate-600"/><div className="mt-1 h-20 w-px bg-gradient-to-b from-cyan-300/50 to-transparent"/><span className="mono [writing-mode:vertical-rl] text-[8px] uppercase tracking-[.26em] text-slate-600">signal rail / ops</span></div>
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/8 site-header backdrop-blur-xl"><div className="container flex h-20 items-center justify-between"><a href="#home" className="flex items-center gap-3"><div className="relative hidden h-10 w-10 overflow-hidden rounded-full border border-cyan-300/35 bg-[#e9eef0] shadow-[0_0_22px_rgba(93,228,255,.12)] sm:block"><img src={asset.profile} alt="Sooraj Poojary profile" className="h-full w-full object-cover object-top"/><span className="absolute bottom-0 right-0 h-2 w-2 rounded-full border-2 border-[#000000] bg-emerald-300"/></div><div><div className="display text-base font-bold tracking-[.26em] text-white">SOORAJ</div><div className="mono text-[9px] tracking-[.2em] text-cyan-300">DEVOPS ENGINEER</div></div></a><nav className="hidden items-center gap-7 lg:flex">{navItems.map(item => <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="mono text-[10px] uppercase tracking-[.14em] text-slate-400 transition-colors hover:text-cyan-300">{item}</button>)}</nav><div className="hidden items-center gap-3 md:flex"><ThemeToggle/><span className="flex items-center gap-2 mono text-[10px] uppercase tracking-[.14em] text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300"/>Available to connect</span><button onClick={() => scrollTo("contact")} className="rounded-lg border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 mono text-[10px] uppercase tracking-[.12em] text-cyan-200 transition hover:bg-cyan-300/20">Let's Connect <ArrowUpRight className="ml-1 inline h-3 w-3"/></button></div><div className="flex items-center gap-2 md:hidden"><ThemeToggle/><button aria-label="Toggle menu" className="rounded-lg border border-white/10 p-2 text-slate-300" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X/> : <Menu/>}</button></div></div>{menuOpen && <div className="border-t border-white/10 site-alt-surface px-5 py-5 md:hidden">{navItems.map(item => <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="block w-full border-b border-white/8 py-4 text-left mono text-xs uppercase tracking-[.14em] text-slate-300">{item}</button>)}</div>}</header>

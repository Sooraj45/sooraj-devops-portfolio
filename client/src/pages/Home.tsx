  ...
  return <div className="glass animate-shimmer rounded-2xl p-5" onClick={() => inputRef.current?.focus()}>
    ...
    <div ref={scrollRef} className="h-48 space-y-1.5 overflow-y-auto mono text-xs pr-1">
      {history.map((line, i) => line.type === "input"
        ? <div key={i} className="flex gap-2"><span className="shrink-0 text-violet-300">sooraj@control-plane:~$</span><span className="text-slate-200">{line.text}</span></div>
        : <div key={i} className="whitespace-pre-wrap pl-0 text-slate-400">{line.text}</div>)}
      <div className="flex items-center gap-2">
        <span className="shrink-0 text-violet-300">sooraj@control-plane:~$</span>
        <input ref={inputRef} value={input} onChange={e => setInput(e.target.value)} onKeyDown={onKeyDown} spellCheck={false} autoComplete="off" aria-label="Terminal input" className="flex-1 bg-transparent text-slate-100 outline-none caret-violet-300"/>
      </div>
    </div>
  </div>;
}
...
export default function Home() {
  ...
  return <div className="min-h-screen overflow-hidden bg-[#000000] text-slate-100"><div aria-hidden="true" className="pointer-events-none fixed left-4 top-1/2 z-40 hidden -translate-y-1/2 flex-col items-center gap-3 xl:flex"><div className="mb-1 h-10 w-px bg-gradient-to-b from-transparent via-violet-300/60 to-transparent"/><span className="h-2 w-2 rounded-full bg-violet-300 shadow-[0_0_15px_rgba(167,139,250,.8)]"/><span className="h-1 w-1 rounded-full bg-slate-600"/><span className="h-1 w-1 rounded-full bg-slate-600"/><span className="h-1 w-1 rounded-full bg-slate-600"/><div className="mt-1 h-20 w-px bg-gradient-to-b from-violet-300/50 to-transparent"/><span className="mono [writing-mode:vertical-rl] text-[8px] uppercase tracking-[.26em] text-slate-600">signal rail / ops</span></div>
    <header className="fixed inset-x-0 top-0 z-50 border-b border-white/8 bg-[#000000]/80 backdrop-blur-xl"><div className="container flex h-20 items-center justify-between"><a href="#home" className="flex items-center gap-3"><div className="relative hidden h-10 w-10 overflow-hidden rounded-full border border-violet-300/35 bg-[#e9eef0] shadow-[0_0_22px_rgba(167,139,250,.12)] sm:block"><img src={asset.profile} alt="Sooraj Poojary profile" className="h-full w-full object-cover object-top"/><span className="absolute bottom-0 right-0 h-2 w-2 rounded-full border-2 border-[#000000] bg-emerald-300"/></div><div><div className="display text-base font-bold tracking-[.26em] text-white">SOORAJ</div><div className="mono text-[9px] tracking-[.2em] text-violet-300">DEVOPS ENGINEER</div></div></a><nav className="hidden items-center gap-7 lg:flex">{navItems.map(item => <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="mono text-[10px] uppercase tracking-[.14em] text-slate-400 transition-colors hover:text-violet-300">{item}</button>)}</nav><div className="hidden items-center gap-3 md:flex"><span className="flex items-center gap-2 mono text-[10px] uppercase tracking-[.14em] text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300"/>Available to connect</span><button onClick={() => scrollTo("contact")} className="rounded-lg border border-violet-300/30 bg-violet-300/10 px-4 py-2 mono text-[10px] uppercase tracking-[.12em] text-violet-200 transition hover:bg-violet-300/20">Let's Connect <ArrowUpRight className="ml-1 inline h-3 w-3"/></button></div><button aria-label="Toggle menu" className="rounded-lg border border-white/10 p-2 text-slate-300 md:hidden" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X/> : <Menu/>}</button></div>{menuOpen && <div className="border-t border-white/10 bg-[#050505] px-5 py-5 md:hidden">{navItems.map(item => <button key={item} onClick={() => scrollTo(item.toLowerCase())} className="block w-full border-b border-white/8 py-4 text-left mono text-xs uppercase tracking-[.14em] text-slate-300">{item}</button>)}</div>}</header>

    <main id="home">
      <section className="relative overflow-hidden pt-28"><DevOpsHero scrollTo={scrollTo}/></section>

      <section id="about" className="relative py-28 md:py-36"><div className="container grid gap-14 lg:grid-cols-[.82fr_1.18fr]"><Reveal><SectionHeading kicker="01 / About" title="Engineering infrastructure with security in mind." copy="A practical DevOps and systems profile shaped by cloud infrastructure, administration, secure connectivity, backup operations and application deployment."/><p className="max-w-xl text-base leading-8 text-slate-400">I bring hands-on experience across cloud infrastructure, systems administration and application deployment, with a focus on AWS EC2 and S3, Oracle Cloud, virtualization, network and firewall security, VPN configuration, backup and disaster recovery, and front-end development. My work supports business-critical infrastructure with minimal downtime.</p></Reveal><Reveal delay={.1}><div className="grid gap-3 sm:grid-cols-2">{["Cloud Infrastructure", "Systems Administration", "Application Deployment", "Security & Networking"].map((item, i) => <div key={item} className="glass group rounded-xl p-6 transition hover:-translate-y-1 hover:border-violet-300/30"><div className="mb-8 flex items-center justify-between"><span className="mono text-[10px] text-slate-500">0{i + 1}</span><ArrowUpRight className="h-4 w-4 text-violet-300 opacity-50 transition group-hover:opacity-100"/></div><div className="display text-xl font-semibold text-white">{item}</div><div className="mt-3 h-px w-14 bg-violet-300/50"/></div>)}</div><div className="mt-4"><TerminalCard/></div></Reveal></div></section>

      <section id="expertise" className="relative border-y border-white/8 bg-[#050505] py-28"><div className="container"><Reveal><SectionHeading kicker="02 / Core expertise" title="The systems behind the delivery." copy="A recruiter-friendly view of the infrastructure domains represented in the resume."/></Reveal><div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{expertise.map(([title, desc, tags, Icon], i) => <Reveal key={title} delay={i * .04}><article className="glass animate-shimmer group h-full rounded-2xl p-6 transition duration-300 hover:-translate-y-2 hover:border-violet-300/35"><div className="flex items-start justify-between"><div className="rounded-xl border border-violet-300/20 bg-violet-300/8 p-3 text-violet-300"><Icon className="h-5 w-5"/></div><span className="mono text-[10px] text-slate-600">DOMAIN / 0{i + 1}</span></div><div className="mt-5 flex items-center justify-between border-y border-white/8 py-2 mono text-[9px] uppercase tracking-[.14em] text-slate-500"><span>surface / operational</span><span className="flex items-center gap-1.5 text-emerald-300"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300"/>active</span></div><h3 className="display mt-8 text-2xl font-semibold text-white">{title}</h3><p className="mt-3 min-h-16 text-sm leading-6 text-slate-400">{desc}</p><div className="mt-6 flex flex-wrap gap-2">{tags.map(tag => <span key={tag} className="rounded-md border border-white/10 bg-white/[.035] px-2.5 py-1.5 mono text-[10px] text-slate-300">{tag}</span>)}</div></article></Reveal>)}</div></div></section>

      <section id="experience" className="py-28 md:py-36"><div className="container"><Reveal><SectionHeading kicker="03 / Experience" title="A career built close to the system." copy="Exact roles and responsibilities from the resume, organized as an operational timeline."/></Reveal><div className="relative max-w-5xl"><div className="absolute left-3 top-0 h-full w-px bg-gradient-to-b from-violet-300/65 via-violet-300/35 to-transparent md:left-5"/>{experience.map((item, i) => <Reveal key={`${item.company}-${item.role}`} delay={i * .04}><article className="relative mb-10 pl-12 md:pl-16"><div className={`absolute left-0 top-1.5 h-7 w-7 rounded-full border-4 border-[#000000] ${item.tone === "cyan" ? "bg-violet-300" : item.tone === "amber" ? "bg-amber-300" : item.tone === "violet" ? "bg-violet-300" : "bg-slate-500"}`}/><div className="glass rounded-2xl p-6 md:p-8"><div className="flex flex-col justify-between gap-3 md:flex-row md:items-start"><div><div className="eyebrow">{item.date}</div><h3 className="display mt-3 text-2xl font-semibold text-white md:text-3xl">{item.role}</h3><div className="mt-1 text-base text-violet-200">{item.company}</div></div><span className="rounded-full border border-white/10 px-3 py-1 mono text-[10px] uppercase tracking-[.12em] text-slate-500">Role {String(i + 1).padStart(2, "0")}</span></div><ul className="mt-7 grid gap-3 md:grid-cols-2">{item.bullets.map(b => <li key={b} className="flex gap-3 text-sm leading-6 text-slate-400"><Check className="mt-1 h-4 w-4 shrink-0 text-violet-300"/>{b}</li>)}</ul>{i === 0 && <div className="mt-7 rounded-xl border border-violet-300/15 bg-violet-300/[.04] p-4 mono text-[10px] uppercase tracking-[.12em] text-slate-400">GoDaddy DNS <ChevronRight className="mx-1 inline h-3 w-3 text-violet-300"/> Instra WAF <ChevronRight className="mx-1 inline h-3 w-3 text-violet-300"/> AWS EC2 <ChevronRight className="mx-1 inline h-3 w-3 text-violet-300"/> Application</div>}</div></article></Reveal>)}</div></div></section>

      <InfrastructureSection/>

      <section id="skills" className="py-28 md:py-36"><div className="container"><Reveal><SectionHeading kicker="05 / Skills matrix" title="The working vocabulary." copy={`${allSkills.length} technologies and practices listed in the resume, grouped by the systems they support.`}/></Reveal><div className="grid gap-4 lg:grid-cols-2">{skillGroups.map(([group, skills], i) => <Reveal key={group} delay={i * .04}><div className="glass animate-shimmer rounded-2xl p-6"><div className="flex items-center justify-between border-b border-white/10 pb-4"><div><div className="mono text-[9px] uppercase tracking-[.18em] text-slate-600">matrix / {String(i + 1).padStart(2, "0")}</div><h3 className="display mt-1 text-xl font-semibold text-white">{group}</h3></div><span className="mono text-[10px] text-violet-300">{String(skills.length).padStart(2, "0")} nodes</span></div><div className="mt-5 flex flex-wrap gap-2">{skills.map(skill => <div key={skill} className="group rounded-lg border border-white/10 bg-white/[.025] px-3 py-2 transition hover:border-violet-300/40 hover:bg-violet-300/[.06]"><span className="mr-2 inline-block h-1.5 w-1.5 rounded-full bg-violet-300/70 transition group-hover:bg-violet-300"/><span className="text-sm text-slate-300">{skill}</span></div>)}</div></div></Reveal>)}</div></div></section>

      <section id="projects" className="border-y border-white/8 bg-[#050505] py-28"><div className="container"><Reveal><SectionHeading kicker="06 / Selected projects" title="Work that ships." copy="A selection of public projects spanning production websites, developer experience and business applications."/></Reveal><div className="grid gap-5 lg:grid-cols-3">{projects.map((project, i) => <Reveal key={project.title} delay={i * .06}><article className="glass group flex h-full flex-col rounded-2xl p-6 transition duration-300 hover:-translate-y-2 hover:border-violet-300/35"><div className="flex items-center justify-between"><span className="mono text-[10px] uppercase tracking-[.16em] text-violet-300">Project {String(i + 1).padStart(2, "0")}</span><Code2 className="h-5 w-5 text-slate-500 transition group-hover:text-violet-300"/></div><h3 className="display mt-8 text-2xl font-semibold text-white">{project.title}</h3><p className="mt-4 flex-1 text-sm leading-7 text-slate-400">{project.description}</p><div className="mt-6 flex flex-wrap gap-2">{project.tags.map(tag => <span key={tag} className="rounded-md border border-white/10 bg-white/[.03] px-2.5 py-1.5 mono text-[9px] text-slate-300">{tag}</span>)}</div><div className="mt-7 flex items-center gap-5 border-t border-white/10 pt-5 mono text-[10px] uppercase tracking-[.12em]"><a href={project.source} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-400 transition hover:text-violet-300"><Github className="h-4 w-4"/>Source</a>{"live" in project && <a href={project.live} target="_blank" rel="noreferrer" className="flex items-center gap-2 text-slate-400 transition hover:text-violet-300"><ExternalLink className="h-4 w-4"/>Live</a>}</div></article></Reveal>)}</div></div></section>

      <section id="security" className="border-y border-white/8 bg-[#050505] py-28"><div className="container grid gap-12 lg:grid-cols-[1fr_.85fr] lg:items-center"><Reveal><div className="eyebrow">06 / Security posture</div><h2 className="display mt-4 max-w-xl text-4xl font-semibold tracking-[-.04em] text-white md:text-6xl">Security is part of the infrastructure.</h2><p className="mt-6 max-w-xl text-lg leading-8 text-slate-400">The resume reflects practical experience across WAF, firewalls, VPN, DNS, endpoint protection and cloud security. This section keeps the claim grounded: secure connectivity and managed controls as part of reliable operations.</p><div className="mt-8 flex flex-wrap gap-2">{["WAF", "Firewall", "VPN", "CloudFlare", "SonicWall", "Sophos", "Kaspersky", "Security Compliance", "Cloud Security", "DNS"].map(tag => <span key={tag} className="rounded-full border border-amber-300/20 bg-amber-300/[.05] px-3 py-2 mono text-[10px] uppercase tracking-[.08em] text-amber-100">{tag}</span>)}</div></Reveal><Reveal delay={.1}><div className="relative mx-auto flex aspect-square max-w-[420px] items-center justify-center rounded-full border border-violet-300/15 bg-[radial-gradient(circle,rgba(167,139,250,.13),transparent_58%)]"><div className="absolute inset-10 rounded-full border border-dashed border-violet-300/25"/><div className="absolute inset-20 rounded-full border border-white/10"/><div className="relative flex h-28 w-28 items-center justify-center rounded-3xl border border-violet-300/40 bg-[#0a0a0a] text-violet-300 shadow-[0_0_60px_rgba(167,139,250,.16)]"><LockKeyhole className="h-10 w-10"/></div>{["WAF", "VPN", "DNS", "Firewall"].map((item, i) => <div key={item} className={`absolute rounded-lg border border-white/10 bg-[#101010] px-3 py-2 mono text-[10px] text-slate-300 ${["top-8 left-1/2 -translate-x-1/2", "right-3 top-1/2 -translate-y-1/2", "bottom-8 left-1/2 -translate-x-1/2", "left-3 top-1/2 -translate-y-1/2"][i]}`}>{item}</div>)}</div></Reveal></div></section>

      <section id="education" className="py-28 md:py-36"><div className="container grid gap-12 lg:grid-cols-[.85fr_1.15fr]"><Reveal><SectionHeading kicker="07 / Education" title="A foundation in information technology."/><div className="glass rounded-2xl p-6"><div className="eyebrow">Certification</div><div className="mt-5 flex items-start gap-4"><div className="rounded-xl bg-amber-300/10 p-3 text-amber-200"><Cloud className="h-6 w-6"/></div><div><h3 className="display text-xl font-semibold text-white">Oracle Cloud Infrastructure</h3><p className="mt-1 text-sm text-slate-400">Certified Foundations Associate · 2025</p></div></div></div></Reveal><Reveal delay={.1}><div className="space-y-4"><div className="glass rounded-2xl p-7"><div className="flex items-start justify-between gap-4"><div><div className="eyebrow">2026</div><h3 className="display mt-3 text-2xl font-semibold text-white">M.Sc. IT</h3><p className="mt-2 text-slate-400">Chandrabhan Sharma College, Mumbai</p></div><span className="mono text-sm text-violet-300">CGPI 9.45</span></div></div><div className="glass rounded-2xl p-7"><div className="flex items-start justify-between gap-4"><div><div className="eyebrow">2023</div><h3 className="display mt-3 text-2xl font-semibold text-white">B.Sc. IT</h3><p className="mt-2 text-slate-400">S.M. Shetty College</p></div><span className="mono text-sm text-violet-300">CGPI 6.95</span></div></div></div></Reveal></div></section>

      <section className="border-y border-white/8 bg-[#050505] py-20"><div className="container grid gap-5 md:grid-cols-2"><Reveal><div className="glass rounded-2xl p-6"><div className="eyebrow">Languages</div><div className="mt-5 grid gap-3 sm:grid-cols-2">{[["English", "Professional Working"], ["Hindi", "Native / Bilingual"], ["Tulu", "Native / Bilingual"], ["Marathi", "Elementary"]].map(([name, level]) => <div key={name} className="flex items-center justify-between border-b border-white/8 pb-3"><span className="text-sm text-slate-200">{name}</span><span className="mono text-[10px] text-slate-500">{level}</span></div>)}</div></div></Reveal><Reveal delay={.08}><div className="glass rounded-2xl p-6"><div className="eyebrow">Interests</div><div className="mt-5 flex flex-wrap gap-2">{["Cricket", "Gaming"].map(item => <span key={item} className="rounded-md border border-white/10 bg-white/[.03] px-3 py-2 mono text-[10px] uppercase tracking-[.12em] text-slate-300">{item}</span>)}</div><p className="mt-7 text-sm leading-6 text-slate-500">A small personal layer from the resume, kept separate from the technical profile.</p></div></Reveal></div></section>

      <section className="border-y border-white/8 bg-[#050505] py-24"><div className="container grid gap-10 lg:grid-cols-[1fr_.8fr] lg:items-center"><Reveal><div className="eyebrow">Beyond infrastructure</div><h2 className="display mt-4 text-4xl font-semibold tracking-[-.04em] text-white md:text-5xl">Reliable systems need clear interfaces.</h2><p className="mt-5 max-w-xl text-lg leading-8 text-slate-400">Alongside infrastructure work, I also contribute to front-end development with HTML, CSS and JavaScript for a fintech application.</p><div className="mt-6 flex gap-2">{["HTML", "CSS", "JavaScript"].map(x => <span key={x} className="rounded-md border border-white/10 px-3 py-2 mono text-[10px] text-violet-200">{x}</span>)}</div></Reveal><Reveal delay={.1}><div className="glass overflow-hidden rounded-2xl"><div className="flex items-center gap-2 border-b border-white/10 px-4 py-3"><span className="h-2 w-2 rounded-full bg-red-300/70"/><span className="h-2 w-2 rounded-full bg-amber-300/70"/><span className="h-2 w-2 rounded-full bg-emerald-300/70"/><span className="ml-auto mono text-[9px] text-slate-500">app-preview.local</span></div><div className="h-48 bg-[radial-gradient(circle_at_70%_30%,rgba(167,139,250,.22),transparent_28%),linear-gradient(135deg,#111111,#080808)] p-6"><div className="h-3 w-24 rounded bg-violet-300/70"/><div className="mt-5 h-2 w-44 rounded bg-white/20"/><div className="mt-2 h-2 w-32 rounded bg-white/10"/><div className="mt-7 flex gap-3"><div className="h-16 w-20 rounded-lg border border-white/10 bg-white/5"/><div className="h-16 w-20 rounded-lg border border-white/10 bg-white/5"/><div className="h-16 w-20 rounded-lg border border-white/10 bg-white/5"/></div></div></div></Reveal></div></section>

      <section className="py-24"><div className="container"><Reveal><div className="glass grid-texture relative overflow-hidden rounded-3xl p-8 md:p-14"><div className="absolute inset-0 bg-[radial-gradient(circle_at_85%_20%,rgba(167,139,250,.14),transparent_30%)]"/><div className="relative flex flex-col justify-between gap-10 md:flex-row md:items-end"><div><div className="eyebrow">Resume / detailed record</div><h2 className="display mt-4 max-w-2xl text-4xl font-semibold tracking-[-.04em] text-white md:text-6xl">Want the complete career record?</h2><p className="mt-5 max-w-xl text-lg leading-8 text-slate-400">Request my latest resume for a detailed overview of professional experience, technical skills, certifications and education.</p></div><div className="flex flex-wrap gap-3"><a href="mailto:soorajpoojary45@gmail.com?subject=Resume%20request" className="rounded-lg bg-violet-300 px-5 py-3.5 display text-sm font-semibold text-[#000000] transition hover:bg-violet-200">Request Resume <Mail className="ml-2 inline h-4 w-4"/></a><button onClick={() => scrollTo("contact")} className="rounded-lg border border-white/15 px-5 py-3.5 display text-sm font-semibold text-white">Contact Me</button></div></div></div></Reveal></div></section>

      <section id="contact" className="border-t border-white/8 bg-[#050505] py-28"><div className="container grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-start"><Reveal><div className="eyebrow">08 / Contact</div><h2 className="display mt-4 max-w-xl text-5xl font-semibold tracking-[-.05em] text-white md:text-7xl">Let's build something reliable.</h2><p className="mt-6 max-w-md text-lg leading-8 text-slate-400">Open to conversations around DevOps, cloud infrastructure, system administration and technology.</p><div className="mt-10 space-y-4"><button onClick={copyEmail} className="flex items-center gap-3 text-left text-slate-300 transition hover:text-violet-300"><Mail className="h-5 w-5 text-violet-300"/>{copied ? "Copied" : email}<Copy className="h-3.5 w-3.5 text-slate-500"/></button><div className="flex items-center gap-3 text-slate-400"><Globe2 className="h-5 w-5 text-violet-300"/>Mumbai, India</div></div></Reveal><Reveal delay={.1}><form onSubmit={e => { e.preventDefault(); toast.success("Thanks — this form is ready to connect to your preferred email workflow."); }} className="glass rounded-2xl p-6 md:p-8"><div className="grid gap-5 md:grid-cols-2"><label className="space-y-2"><span className="mono text-[10px] uppercase tracking-[.14em] text-slate-500">Name</span><input required className="w-full rounded-lg border border-white/10 bg-white/[.04] px-4 py-3 text-sm text-white outline-none transition focus:border-violet-300/50" placeholder="Your name"/></label><label className="space-y-2"><span className="mono text-[10px] uppercase tracking-[.14em] text-slate-500">Email</span><input required type="email" className="w-full rounded-lg border border-white/10 bg-white/[.04] px-4 py-3 text-sm text-white outline-none transition focus:border-violet-300/50" placeholder="you@company.com"/></label></div><label className="mt-5 block space-y-2"><span className="mono text-[10px] uppercase tracking-[.14em] text-slate-500">Subject</span><input required className="w-full rounded-lg border border-white/10 bg-white/[.04] px-4 py-3 text-sm text-white outline-none transition focus:border-violet-300/50" placeholder="A reliable opportunity"/></label><label className="mt-5 block space-y-2"><span className="mono text-[10px] uppercase tracking-[.14em] text-slate-500">Message</span><textarea required rows={5} className="w-full resize-none rounded-lg border border-white/10 bg-white/[.04] px-4 py-3 text-sm text-white outline-none transition focus:border-violet-300/50" placeholder="Tell me a little about the systems or team..."/></label><button type="submit" className="mt-6 rounded-lg bg-violet-300 px-5 py-3.5 display text-sm font-semibold text-[#000000] transition hover:bg-violet-200 active:scale-[.98]">Send Message <ArrowUpRight className="ml-2 inline h-4 w-4"/></button></form></Reveal></div></section>
    </main>

    <footer className="border-t border-white/8 bg-[#000000] py-10"><div className="container flex flex-col justify-between gap-6 md:flex-row md:items-center"><div><div className="display text-sm font-bold tracking-[.18em] text-white">SOORAJ POOJARY</div><div className="mt-1 mono text-[9px] tracking-[.18em] text-violet-300">CLOUD · INFRASTRUCTURE · SECURITY · AUTOMATION</div></div><div className="flex items-center gap-5 mono text-[10px] uppercase tracking-[.12em] text-slate-500"><a href="#home" className="hover:text-violet-300">Home</a><a href="#experience" className="hover:text-violet-300">Experience</a><a href="#skills" className="hover:text-violet-300">Skills</a><a href="#contact" className="hover:text-violet-300">Contact</a></div><div className="mono text-[10px] text-slate-600">© 2026 · Built for reliable systems</div></div></footer>
  </div>;
}

    ...
    <div className="space-y-2">
      {infrastructureSteps.map((step, i) => {
        ...
        return (
          ...
          >
            <span className={`h-2 w-2 shrink-0 rounded-full transition-all duration-300 group-hover:scale-125 ${amber ? "bg-amber-300 group-hover:shadow-[0_0_12px_rgba(246,201,106,.8)]" : "bg-violet-300 group-hover:shadow-[0_0_12px_rgba(167,139,250,.8)]"}`} />
            <span className="mono w-24 shrink-0 text-xs tracking-[.18em] text-slate-300">{step.title}</span>
            <span className="h-px w-8 shrink-0 bg-white/15 transition-all duration-300 group-hover:w-12 group-hover:bg-violet-300/40" />

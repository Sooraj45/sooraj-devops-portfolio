    ...
    <div className="relative ml-[21.5px] h-6 w-px">
      <svg className="absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 2 24" preserveAspectRatio="none" aria-hidden="true">
        <motion.line
          ...
          stroke={active ? "#a78bfa" : "rgba(167,139,250,.32)"}
          strokeWidth="1.5"
      ...
      <span className={`animate-route absolute left-1/2 top-0 h-2 w-[3px] -translate-x-1/2 rounded-full bg-white shadow-[0_0_10px_#a78bfa] transition-opacity duration-300 ${active ? "opacity-100" : "opacity-60"}`} />
    </div>
  );
}
